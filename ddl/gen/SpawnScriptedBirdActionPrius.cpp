...
namespace rivet::ddl::generated {
	SpawnScriptedBirdActionPrius::SpawnScriptedBirdActionPrius([[maybe_unused]] const std::shared_ptr<const rivet::structures::rivet_serialized_object> &serialized): rivet::structures::rivet_ddl_base(serialized) {
		...
		WaitAllSpawned = serialized->get_bool(WaitAllSpawned_type_id);
		MinSeparation = serialized->get_float(MinSeparation_type_id);
		FlockSize = serialized->get_int32(FlockSize_type_id);
		FleeDistance = serialized->get_float(FleeDistance_type_id);
		FleeOnNearbyCombat = serialized->get_bool(FleeOnNearbyCombat_type_id);
