// <auto-generated>
// SPDX-FileCopyrightText: 2025 Legiayayana
//
// SPDX-License-Identifier: EUPL-1.2

namespace Rivet.DDL.Types;

#nullable enable

using Rivet.Models;
using Rivet.DDL.Enums;

[DDLRegistration(0xf01a2056u)]
public class ProjectileFlyingStopEvent : EventBase, IDDLObjectType<ProjectileFlyingStopEvent> {
	public new static RivetTypeId TypeId { get; } = new RivetTypeId(0xf01a2056u);

	public ProjectileFlyingStopEvent(DDLObject ddl) : base(ddl) {
		WeaponTypeName = ddl.GetString(0xcb3c1ce8) ?? WeaponTypeName;
		PerkName = ddl.GetString(0x9da07479) ?? PerkName;
		VersionIndex = ddl.GetValue<byte>(0x6e091c9au, VersionIndex);
	}

	[DDLRegistration(0xcb3c1ce8u)]
	public string? WeaponTypeName { get; set; } = default;

	[DDLRegistration(0x9da07479u)]
	public string? PerkName { get; set; } = default;

	[DDLRegistration(0x6e091c9au)]
	public byte VersionIndex { get; set; } = 0x00;

	public new static ProjectileFlyingStopEvent Create(DDLObject ddl) => new(ddl);
}

