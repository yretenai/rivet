...
%fields%
		...
		[[nodiscard]] static auto
		from_substruct(rivet_type_id incoming_type_id) -> std::shared_ptr<%struct_name%>;
	};
} // namespace rivet::ddl::generated
...
namespace rivet::ddl::generated {
	...
	const static %enum_value_type% %enum_name%_values = {
%decl_fields%
	};
...
%field_init%
	...
	auto
	%struct_name%::get_type_id() const noexcept -> rivet::rivet_type_id {
		return type_id;
	}
...
%substructs_1%
	...
	[[nodiscard]] auto
	%struct_name%::from_substruct(rivet_type_id incoming_type_id) -> std::shared_ptr<%struct_name%> {
%substructs_2%
		return nullptr;
	}
} // namespace rivet::ddl::generated
...
std::string_view template_struct_field_value = R"(serialized->get_field<%field_type%>(%field_name%_type_id))";
...
std::string_view template_struct_field_array = R"(serialized->get_fields<%field_type%>(%field_name%_type_id))";
...
std::string_view template_struct_substruct_1 = R"(		if (incoming_type_id == %struct_name%::type_id) {
			return std::make_shared<%struct_name%>(serialized);
		}
)";

std::string_view template_struct_substruct_2 = R"(		if (incoming_type_id == %struct_name%::type_id) {
			return std::make_shared<%struct_name%>();
		}
)";

std::string_view template_struct_substruct_1_recur = R"(		auto %struct_name%_ptr = %struct_name%::from_substruct(incoming_type_id, serialized);
		if (%struct_name%_ptr != nullptr) {
			return %struct_name%_ptr;
		}
)";

std::string_view template_struct_substruct_2_recur = R"(		auto %struct_name%_ptr = %struct_name%::from_substruct(incoming_type_id);
		if (%struct_name%_ptr != nullptr) {
			return %struct_name%_ptr;
		}
)";
