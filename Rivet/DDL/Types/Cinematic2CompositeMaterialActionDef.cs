// <auto-generated>
// SPDX-FileCopyrightText: 2025 Legiayayana
//
// SPDX-License-Identifier: EUPL-1.2

namespace Rivet.DDL.Types;

#nullable enable

using Rivet.Models;
using Rivet.DDL.Enums;

[DDLRegistration(0x0bb8e0d6u)]
public class Cinematic2CompositeMaterialActionDef : DDLObjectType, IDDLObjectType<Cinematic2CompositeMaterialActionDef> {
	public static RivetTypeId TypeId { get; } = new RivetTypeId(0x0bb8e0d6u);

	public Cinematic2CompositeMaterialActionDef(DDLObject ddl) : base(ddl) {
		Id = ddl.GetValue<RivetAssetId>(0x8a514a8du, Id);
		Time = ddl.GetValue<float>(0x5f0458f5u, Time);
		DurationType = ddl.GetEnum<x29c933e2>(0x41e2529fu, x29c933e2Values.Lookup);
		Duration = ddl.GetValue<float>(0x7c5e3db0u, Duration);
		MaterialAsset = ddl.GetValue<RivetAssetId>(0xf55a8ae3u, MaterialAsset);
	}

	[DDLRegistration(0x0bb8e0d6u)]
	public RivetAssetId Id { get; set; } = new RivetAssetId(0x0000000000000000);

	[DDLRegistration(0x0bb8e0d6u)]
	public float Time { get; set; } = 0.00f;

	[DDLRegistration(0x0bb8e0d6u)]
	public x29c933e2 DurationType { get; set; } = x29c933e2.None;

	[DDLRegistration(0x0bb8e0d6u)]
	public float Duration { get; set; } = 0.00f;

	[DDLRegistration(0x0bb8e0d6u)]
	public RivetAssetId MaterialAsset { get; set; } = default;

	public static Cinematic2CompositeMaterialActionDef Create(DDLObject ddl) => new(ddl);
}

