...
namespace rivet::data {
	config_type::config_type(const std::shared_ptr<const structures::rivet_serialized_object> &serialized): rivet_ddl_base(serialized) {
		type = serialized->get_field<std::string_view>(type_type_id);
	}

	auto
	config_type::from_substruct(const rivet_type_id type_id, const std::shared_ptr<const structures::rivet_serialized_object> &serialized) -> std::shared_ptr<rivet_ddl_base> {
	...
	config::config(const std::shared_ptr<rivet_data_array> &stream): data(stream) {
		...
		const auto asset_refs = data.get_section<structures::rivet_ref>(config_refs_type_id);
		...
		if (type_blob == nullptr) {
			throw invalid_operation("config::config: invalid stream");
		}
		...
		const auto type_serialized = std::make_shared<ddl::serialized>(type_blob, data.buffer);
		value = std::make_shared<ddl::serialized>(data_blob, data.buffer);
