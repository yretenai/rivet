// <auto-generated>
// SPDX-FileCopyrightText: 2025 Legiayayana
//
// SPDX-License-Identifier: EUPL-1.2

namespace Rivet.DDL.Types;

#nullable enable

using Rivet.Models;
using Rivet.DDL.Enums;

[DDLRegistration(0x45d1831eu)]
public class Cinematic2CameraDofFollowTrackDef : DDLObjectType, IDDLObjectType<Cinematic2CameraDofFollowTrackDef> {
	public static RivetTypeId TypeId { get; } = new RivetTypeId(0x45d1831eu);

	public Cinematic2CameraDofFollowTrackDef(DDLObject ddl) : base(ddl) {
		Id = ddl.GetValue<RivetAssetId>(0x8a514a8du, Id);
		Name = ddl.GetString(0x6eb3beb6) ?? Name;
		PlatformFilter = ddl.GetBitset<xb52df7e7>(0x92542397u, xb52df7e7Values.Lookup);
		Tags = ddl.GetObject<Cinematic2TrackTagsDef>(0x5f2c5496u);
		Global = ddl.GetValue<bool>(0xd67dc5e4u, Global);
		FollowActions = ddl.GetDictionary<RivetAssetId, Cinematic2CameraDofFollowActionDef>(0x271e731d, DDLMapTypeHandler.VisitRivetAssetId, (mapId, mapDDL) => mapDDL.GetObject<Cinematic2CameraDofFollowActionDef>(mapId));
	}

	[DDLRegistration(0x45d1831eu)]
	public RivetAssetId Id { get; set; } = new RivetAssetId(0x0000000000000000);

	[DDLRegistration(0x45d1831eu)]
	public string? Name { get; set; } = default;

	[DDLRegistration(0x45d1831eu, description: "Track will only run on the defined platform(s)")]
	public xb52df7e7 PlatformFilter { get; set; } = (xb52df7e7) 0x00000007;

	[DDLRegistration(0x45d1831eu, description: "A set of gameplay-defined values to determine if this track should be processed.")]
	public Cinematic2TrackTagsDef? Tags { get; set; } = default;

	[DDLRegistration(0x45d1831eu, description: "If set to true, actions on this track will not move when shots are reordered.")]
	public bool Global { get; set; } = false;

	[DDLRegistration(0x45d1831eu)]
	public Dictionary<RivetAssetId, Cinematic2CameraDofFollowActionDef?> FollowActions { get; set; } = [];

	public static Cinematic2CameraDofFollowTrackDef Create(DDLObject ddl) => new(ddl);
}

