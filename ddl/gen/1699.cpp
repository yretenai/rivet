...
namespace rivet::ddl::generated {
	MountAbilitySpeedBrake::MountAbilitySpeedBrake([[maybe_unused]] const std::shared_ptr<const rivet::structures::rivet_serialized_object> &serialized): MountAbilityBase(serialized) {
		BrakeSpeed = serialized->get_float(BrakeSpeed_type_id);
		BrakeIntro = serialized->get_string(BrakeIntro_type_id);
		BrakeLoop = serialized->get_string(BrakeLoop_type_id);
		BrakeOutro = serialized->get_string(BrakeOutro_type_id); 
	}

	auto
	MountAbilitySpeedBrake::get_type_id() const noexcept -> rivet::rivet_type_id {
		return type_id;
	}
