// <auto-generated>
// SPDX-FileCopyrightText: 2025 Legiayayana
//
// SPDX-License-Identifier: EUPL-1.2

namespace Rivet.DDL.Types;

#nullable enable

using Rivet.IO;
using Rivet.DDL.Enums;

[DDLRegistration(0x9afb259fu)]
public class MagShieldWeaponFiredEvent : WeaponTriggeredEvent, IDDLObjectType<MagShieldWeaponFiredEvent> {
	public new static RivetTypeId TypeId { get; } = new RivetTypeId(0x9afb259fu);

	public MagShieldWeaponFiredEvent(DDLObject ddl) : base(ddl) {
		Red = ddl.GetValue<float>(0xd0347646u, Red);
		Green = ddl.GetValue<float>(0xc9c922e0u, Green);
		Blue = ddl.GetValue<float>(0xaea60a04u, Blue);
	}

	[DDLRegistration(0xd0347646u)]
	public float Red { get; set; } = 0.00f;

	[DDLRegistration(0xc9c922e0u)]
	public float Green { get; set; } = 0.00f;

	[DDLRegistration(0xaea60a04u)]
	public float Blue { get; set; } = 0.00f;

	public new static MagShieldWeaponFiredEvent Create(DDLObject ddl) => new(ddl);
}

