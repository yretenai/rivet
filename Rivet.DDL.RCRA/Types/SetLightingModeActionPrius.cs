// <auto-generated>
// SPDX-FileCopyrightText: 2025-2026 Neptuwunium
//
// SPDX-License-Identifier: EUPL-1.2

namespace Rivet.DDL.Types;

#nullable enable

using Rivet.IO;
using Rivet.DDL.Enums;

[DDLRegistration(0x07b56c22u)]
public class SetLightingModeActionPrius : DDLObjectType, IDDLObjectType<SetLightingModeActionPrius> {
	public static RivetTypeId TypeId { get; } = new RivetTypeId(0x07b56c22u);

	public SetLightingModeActionPrius(DDLObject ddl) : base(ddl) {
		ActionType = ddl.GetEnum<x10228b34>(0x874b2194u, x10228b34Values.Lookup);
		IsSaved = ddl.GetValue<bool>(0xf11ec123u, IsSaved);
	}

	[DDLRegistration(0x874b2194u)]
	public x10228b34 ActionType { get; set; } = x10228b34.Loading;

	[DDLRegistration(0xf11ec123u)]
	public bool IsSaved { get; set; } = true;

	public static SetLightingModeActionPrius Create(DDLObject ddl) => new(ddl);
}

