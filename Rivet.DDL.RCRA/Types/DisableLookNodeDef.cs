// <auto-generated>
// SPDX-FileCopyrightText: 2025-2026 Neptuwunium
//
// SPDX-License-Identifier: EUPL-1.2

namespace Rivet.DDL.Types;

#nullable enable

using Rivet.IO;
using Rivet.DDL.Enums;

[DDLRegistration(0xc9f63bf5u)]
public class DisableLookNodeDef : DDLObjectType, IDDLObjectType<DisableLookNodeDef> {
	public static RivetTypeId TypeId { get; } = new RivetTypeId(0xc9f63bf5u);

	public DisableLookNodeDef(DDLObject ddl) : base(ddl) {
		Id = ddl.GetValue<RivetAssetId>(0x8a514a8du, Id);
		Type = ddl.GetEnum<x577f103e>(0xbc4e9799u, x577f103eValues.Lookup);
		Collapsed = ddl.GetValue<bool>(0xf07e7fafu, Collapsed);
		Disabled = ddl.GetValue<bool>(0x7effd393u, Disabled);
		X = ddl.GetValue<float>(0x5e33418du, X);
		Y = ddl.GetValue<float>(0x2934711bu, Y);
		Input = ddl.GetObject<InputPlugSignal>(0xc17bfe16u);
		LookName = ddl.GetString(0xad8c1e16) ?? LookName;
	}

	[DDLRegistration(0x8a514a8du)]
	public RivetAssetId Id { get; set; } = new RivetAssetId(0x0000000000000000);

	[DDLRegistration(0xbc4e9799u)]
	public x577f103e Type { get; set; } = x577f103e.Invalid;

	[DDLRegistration(0xf07e7fafu)]
	public bool Collapsed { get; set; } = false;

	[DDLRegistration(0x7effd393u)]
	public bool Disabled { get; set; } = false;

	[DDLRegistration(0x5e33418du)]
	public float X { get; set; } = 0.00f;

	[DDLRegistration(0x2934711bu)]
	public float Y { get; set; } = 0.00f;

	[DDLRegistration(0xc17bfe16u)]
	public InputPlugSignal? Input { get; set; } = default;

	[DDLRegistration(0xad8c1e16u)]
	public string? LookName { get; set; } = default;

	public static DisableLookNodeDef Create(DDLObject ddl) => new(ddl);
}

