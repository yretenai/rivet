// <auto-generated>
// SPDX-FileCopyrightText: 2025 Legiayayana
//
// SPDX-License-Identifier: EUPL-1.2

namespace Rivet.DDL.Types;

#nullable enable

using Rivet.Models;
using Rivet.DDL.Enums;

[DDLRegistration(0xaa32b636u)]
public class ModelSplineColorRandomness : DDLObjectType, IDDLObjectType<ModelSplineColorRandomness> {
	public static RivetTypeId TypeId { get; } = new RivetTypeId(0xaa32b636u);

	public ModelSplineColorRandomness(DDLObject ddl) : base(ddl) {
		X = ddl.GetValue<float>(0x5e33418du, X);
		Y = ddl.GetValue<float>(0x2934711bu, Y);
		Z = ddl.GetValue<float>(0xb03d20a1u, Z);
	}

	[DDLRegistration(0xaa32b636u, "Lunimance")]
	public float X { get; set; } = 0.00f;

	[DDLRegistration(0xaa32b636u, "Bias")]
	public float Y { get; set; } = 0.00f;

	[DDLRegistration(0xaa32b636u, "Hue")]
	public float Z { get; set; } = 0.00f;

	public static ModelSplineColorRandomness Create(DDLObject ddl) => new(ddl);
}

