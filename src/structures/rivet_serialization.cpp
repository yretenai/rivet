...
namespace rivet::structures {
	...
	[[nodiscard]] auto
	rivet_serialized_object::construct(const rivet_type_id type_id) const noexcept -> std::shared_ptr<rivet_ddl_base> {
	...
	[[nodiscard]] auto
	rivet_serialized_object::get_uint64s(const rivet_type_id field_id) const noexcept -> std::vector<uint64_t> {
		return get_fields<uint64_t>(field_id);
	}

	[[nodiscard]] auto
	rivet_serialized_object::get_uint32s(const rivet_type_id field_id) const noexcept -> std::vector<uint32_t> {
		return get_fields<uint64_t, uint32_t>(field_id);
	}

	[[nodiscard]] auto
	rivet_serialized_object::get_uint16s(const rivet_type_id field_id) const noexcept -> std::vector<uint16_t> {
		return get_fields<uint64_t, uint16_t>(field_id);
	}

	[[nodiscard]] auto
	rivet_serialized_object::get_uint8s(const rivet_type_id field_id) const noexcept -> std::vector<uint8_t> {
		return get_fields<uint64_t, uint8_t>(field_id);
	}

	[[nodiscard]] auto
	rivet_serialized_object::get_int64s(const rivet_type_id field_id) const noexcept -> std::vector<int64_t> {
		return get_fields<int64_t>(field_id);
	}

	[[nodiscard]] auto
	rivet_serialized_object::get_int32s(const rivet_type_id field_id) const noexcept -> std::vector<int32_t> {
		return get_fields<int64_t, int32_t>(field_id);
	}

	[[nodiscard]] auto
	rivet_serialized_object::get_int16s(const rivet_type_id field_id) const noexcept -> std::vector<int16_t> {
		return get_fields<int64_t, int16_t>(field_id);
	}

	[[nodiscard]] auto
	rivet_serialized_object::get_int8s(const rivet_type_id field_id) const noexcept -> std::vector<int8_t> {
		return get_fields<int64_t, int8_t>(field_id);
	}

	[[nodiscard]] auto
	rivet_serialized_object::get_doubles(const rivet_type_id field_id) const noexcept -> std::vector<double> {
		return get_fields<double>(field_id);
	}

	[[nodiscard]] auto
	rivet_serialized_object::get_floats(const rivet_type_id field_id) const noexcept -> std::vector<float> {
		return get_fields<double, float>(field_id);
	}

	[[nodiscard]] auto
	rivet_serialized_object::get_bools(const rivet_type_id field_id) const noexcept -> std::vector<bool> {
		return get_fields<bool>(field_id);
	}

	[[nodiscard]] auto
	rivet_serialized_object::get_strings(const rivet_type_id field_id) const noexcept -> std::vector<std::string_view> {
	...
	rivet_serialized_object::to_nlohmann_json() const -> nlohmann::json {
		...
		for (const auto &[field_id, field_value] : values) {
			...
			if (field_value.size() == 1) {
				...
				std::visit(
					[&json, field_name]<typename T0>(T0 &arg) {
