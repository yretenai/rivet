...
namespace rivet::gui {
	std::shared_ptr<std::string> g_status = nullptr;
	std::mutex g_status_mutex;

	void
	set_log_message(const std::string &message) {
		// ReSharper disable once CppDFAUnusedValue
		const std::lock_guard lock(g_status_mutex);
		g_status = std::make_shared<std::string>(message);
	}
	...
	namespace device {
		...
		rivet_sdl_host::rivet_sdl_host()
			...
			flags.FontGlobalScale = 1.5f;
			flags.DisplayFramebufferScale = ImVec2(2.0, 2.0f);
		...
		rivet_sdl_host::render() {
			...
			if (ImGui::BeginViewportSideBar("##MainStatusBar",
				...
				if (ImGui::BeginMenuBar()) {
					ImGui::Text("%s", g_status != nullptr ? g_status->data() : "nothing to report");
					ImGui::EndMenuBar();
				}
				ImGui::End();
			}
