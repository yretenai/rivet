...
namespace rivet::ddl::generated {
	JobManagerFormationPrius::JobManagerFormationPrius([[maybe_unused]] const std::shared_ptr<const rivet::structures::rivet_serialized_object> &serialized): rivet::structures::rivet_ddl_base(serialized) {
		ExclusionVolumeGroup = serialized->get_uint64(ExclusionVolumeGroup_type_id);
		MasterBoundsVolume = serialized->get_uint64(MasterBoundsVolume_type_id);
		OverrideTarget = serialized->get_uint64(OverrideTarget_type_id);
		IsCameraBased = serialized->get_bool(IsCameraBased_type_id);
		MinTimeBetweenFormations = serialized->get_float(MinTimeBetweenFormations_type_id);
		IsTurretBased = serialized->get_bool(IsTurretBased_type_id);
		TurretActor = serialized->get_uint64(TurretActor_type_id);
		EdgeAngle = serialized->get_float(EdgeAngle_type_id); 
	}
