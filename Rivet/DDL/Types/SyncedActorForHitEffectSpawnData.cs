// <auto-generated>
// SPDX-FileCopyrightText: 2025 Legiayayana
//
// SPDX-License-Identifier: EUPL-1.2

namespace Rivet.DDL.Types;

#nullable enable

using Rivet.Models;
using Rivet.DDL.Enums;

[DDLRegistration(0xd52d001au)]
public class SyncedActorForHitEffectSpawnData : ActorSpawnData, IDDLObjectType<SyncedActorForHitEffectSpawnData> {
	public new static RivetTypeId TypeId { get; } = new RivetTypeId(0xd52d001au);

	public SyncedActorForHitEffectSpawnData(DDLObject ddl) : base(ddl) {
		ShotOwner = ddl.GetValue<uint>(0x8b1a8202u, ShotOwner);
		HitActor = ddl.GetValue<uint>(0x51261116u, HitActor);
		AssetId = ddl.GetValue<RivetAssetId>(0xb6e788d7u, AssetId);
		Pos = ddl.GetObject<DDLVector3>(0xaa8ccf65u);
		Facing = ddl.GetObject<DDLVector3>(0xb37847eeu);
		Up = ddl.GetObject<DDLVector3>(0x76fcc3adu);
		MatchCreatorZone = ddl.GetValue<bool>(0xfc7075c0u, MatchCreatorZone);
		MatchCreatorAllegiance = ddl.GetValue<bool>(0x60d5eba5u, MatchCreatorAllegiance);
	}

	[DDLRegistration(0xd52d001au, description: "The shot owner/root/source. The shooter.")]
	public uint ShotOwner { get; set; } = 0x00000000;

	[DDLRegistration(0xd52d001au, description: "The actor we hit, if we hit one.")]
	public uint HitActor { get; set; } = 0x00000000;

	[DDLRegistration(0xd52d001au, description: "AssetId for the asset we're spawning")]
	public RivetAssetId AssetId { get; set; } = new RivetAssetId(0x0000000000000000);

	[DDLRegistration(0xd52d001au)]
	public DDLVector3? Pos { get; set; } = default;

	[DDLRegistration(0xd52d001au)]
	public DDLVector3? Facing { get; set; } = default;

	[DDLRegistration(0xd52d001au)]
	public DDLVector3? Up { get; set; } = default;

	[DDLRegistration(0xd52d001au)]
	public bool MatchCreatorZone { get; set; } = true;

	[DDLRegistration(0xd52d001au)]
	public bool MatchCreatorAllegiance { get; set; } = true;

	public new static SyncedActorForHitEffectSpawnData Create(DDLObject ddl) => new(ddl);
}

