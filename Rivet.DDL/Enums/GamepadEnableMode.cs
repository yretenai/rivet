// <auto-generated>
// SPDX-FileCopyrightText: 2025 Legiayayana
//
// SPDX-License-Identifier: EUPL-1.2

namespace Rivet.DDL.Enums;

[DDLRegistration(0xcfd4e774u)]
public enum GamepadEnableMode : uint {
	[DDLRegistration(0xf7e36f62u)] Never = 0x0u,
	[DDLRegistration(0xca40054bu)] Always = 0x1u,
	[DDLRegistration(0x9a48e8c3u)] ApplicationFocus = 0x2u,
	[DDLRegistration(0xbb152871u)] k3DViewFocus = 0x3u,
}

public static class GamepadEnableModeValues {
	public static Dictionary<string, GamepadEnableMode> Lookup = new() {
		["kNever"] = GamepadEnableMode.Never,
		["kAlways"] = GamepadEnableMode.Always,
		["kApplicationFocus"] = GamepadEnableMode.ApplicationFocus,
		["k3DViewFocus"] = GamepadEnableMode.k3DViewFocus,
	};

	public static Dictionary<GamepadEnableMode, uint> ReverseLookup = new() {
		[GamepadEnableMode.Never] = 0xf7e36f62u,
		[GamepadEnableMode.Always] = 0xca40054bu,
		[GamepadEnableMode.ApplicationFocus] = 0x9a48e8c3u,
		[GamepadEnableMode.k3DViewFocus] = 0xbb152871u,
	};

	public static uint TypeId => 0xcfd4e774u;
}
