// <auto-generated>
// SPDX-FileCopyrightText: 2025 Legiayayana
//
// SPDX-License-Identifier: EUPL-1.2

namespace Rivet.DDL.Types;

#nullable enable

using Rivet.Models;
using Rivet.DDL.Enums;

[DDLRegistration(0x1139b00fu)]
public class MusicCueListenerActionPrius : DDLObjectType, IDDLObjectType<MusicCueListenerActionPrius> {
	public static RivetTypeId TypeId { get; } = new RivetTypeId(0x1139b00fu);

	public MusicCueListenerActionPrius(DDLObject ddl) : base(ddl) {
		ActionType = ddl.GetEnum<x10228b34>(0x874b2194u, x10228b34Values.Lookup);
		CueString = ddl.GetString(0x14f4dc49) ?? CueString;
	}

	[DDLRegistration(0x874b2194u)]
	public x10228b34 ActionType { get; set; } = x10228b34.Sound;

	[DDLRegistration(0x14f4dc49u)]
	public string? CueString { get; set; } = "end_of_song";

	public static MusicCueListenerActionPrius Create(DDLObject ddl) => new(ddl);
}

