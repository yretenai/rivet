// <auto-generated>
// SPDX-FileCopyrightText: 2025 Legiayayana
//
// SPDX-License-Identifier: EUPL-1.2

namespace Rivet.DDL.Types;

#nullable enable

using Rivet.IO;
using Rivet.DDL.Enums;

[DDLRegistration(0xd26811e9u)]
public class SlopeSlideEndEvent : EventBase, IDDLObjectType<SlopeSlideEndEvent> {
	public new static RivetTypeId TypeId { get; } = new RivetTypeId(0xd26811e9u);

	public SlopeSlideEndEvent(DDLObject ddl) : base(ddl) {
		Material = ddl.GetEnum<x10b3c4cf>(0x86bfc8e5u, x10b3c4cfValues.Lookup);
	}

	[DDLRegistration(0x86bfc8e5u)]
	public x10b3c4cf Material { get; set; } = x10b3c4cf.None;

	public new static SlopeSlideEndEvent Create(DDLObject ddl) => new(ddl);
}

